...
//double Mosquito::_fDeathProbability[MAXAGE] = 
...
double Mosquito::_fAgeDistribution[MAXAGE] =   // cumulative density of mosquito ages
...
Mosquito::Mosquito(gsl_rng *rng, Location *p, int nSerotype, int nInfectedAtID) {
  assert(nSerotype>0 && nSerotype<=4);
  _nID = _nNextID++;
  _bDead = false;
  _nSerotype = nSerotype;
  _nInfectedAtID = nInfectedAtID;
  double r = gsl_rng_uniform(rng);
  _nAgeInfected = 0;
  while (_fAgeDistribution[_nAgeInfected]<r && _nAgeInfected<MAXAGE-1)
    _nAgeInfected++;
  _nAgeDeath = _nAgeInfected;
  r = 1.0-(gsl_rng_uniform(rng)*(1.0-_fAgeDistribution[_nAgeDeath]));
  while (_fAgeDistribution[_nAgeDeath]<r && _nAgeDeath<MAXAGE-1)
    _nAgeDeath++;
