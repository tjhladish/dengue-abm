...
void Community::vaccinate(CatchupVaccinationEvent cve) {
    ...
    for (Person* p: _personAgeCohort[cve.age]) {
        assert(p != NULL);
        if (!p->isVaccinated() and p->isSeroEligible(_par->vaccineSeroConstraint) and gsl_rng_uniform(RNG) < cve.coverage) {
...
void Community::targetVaccination(Person* p) {
    ...
    if (p->getAge()==_par->vaccineTargetAge and not p->isVaccinated() and p->isSeroEligible(_par->vaccineSeroConstraint)) {
