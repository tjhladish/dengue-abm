...
const string pop_dir = HOME_DIR + "/work/dengue-abm/pop-" + SIM_POP;
...
vector<double> tally_counts(const Parameters* par, Community* community, int pre_intervention_output) {
    ...
    vector<double> metrics(2*num_years, 0.0);
    ...
    for (int s=0; s<NUM_OF_SEROTYPES; s++) {
        for (int y = 0; y<num_years; ++y) {
            ...
            metrics[num_years + y] += i_tally[s][y];
        }
    }
...
//exit(-10);
    ...
    // pick a starting immunity that is somewhere close to where we expect to end up
    string imm_filename = "/ufrc/longini/tjhladish/"; 
    if (foi_mult < 0.75) {
        imm_filename += "imm_1000_yucatan-alt_foi/immunity2130." + process_id + ".foi0.500000";
    } else if (foi_mult < 1.25) {
        imm_filename += "imm_1000_yucatan-irs_refit3/immunity2030." + process_id;
    } else {
        imm_filename += "imm_1000_yucatan-alt_foi/immunity2130." + process_id + ".foi1.500000";
    }

    par->immunityFilename = imm_filename;
