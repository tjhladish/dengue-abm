...
args <- c("baseline.rds","lagged_intervention.rds","lag_effectiveness.rds","lag_plot.png")
...
scenkeys <- grep("particle|replicate",key(intervention.dt),invert = T, value = T)
...
ggplot(inte.inc) + aes(x=year, y=med/yucpop) +
  facet_grid(catchup ~ vac_first) + geom_line()
