...
//  7 beta_multiplier real,
    ...
    vector<double> abc_args(&args[0], &args[6]);
    string argstring;
...
vector<double> tally_counts(Parameters* par, Community* community) {
    ...
    vector< vector<double> > mild_attack_rate_hosp_y2   = {{0,0},{0,0},{0,0}};
    vector< vector<double> > severe_attack_rate_hosp_y2 = {{0,0},{0,0},{0,0}};
    ...
    for (int i=community->getNumPerson()-1; i>=0; i--) {
        ...
        if (age_at_vac >= age_bins.front()[0] and age_at_vac <= age_bins.back()[1]) {
            ...
            for (const Infection* infec: p->getInfectionHistory()) {
                ...
                } else if (itime >= VAC_DAY + (365*3) + 28 and itime < VAC_DAY + (365*4) + 28) {
                    // hospital phase
                    if ((const bool) infec->isSevere()) {
                        ++severe_attack_rate_hosp_y2[age_class][vaccinated]; 
                    } else {
                        ++mild_attack_rate_hosp_y2[age_class][vaccinated]; 
                    }
                }
            }
        }
    ...
    for (unsigned int i = 0; i < mild_attack_rate_hosp_y2.size(); ++i) {
        for (unsigned int j = 0; j < mild_attack_rate_hosp_y2[i].size(); ++j) metrics.push_back(mild_attack_rate_hosp_y2[i][j] / arm_pop_sizes[i][j]);
    }
    for (unsigned int i = 0; i < severe_attack_rate_hosp_y2.size(); ++i) {
        for (unsigned int j = 0; j < severe_attack_rate_hosp_y2[i].size(); ++j) metrics.push_back(severe_attack_rate_hosp_y2[i][j] / arm_pop_sizes[i][j]);
    }
