...
double __alpha_irs_expr (double rho_par, void *params) {
    ...
    cerr << "params, rho_params ptrs " << params << " " << rho_params << endl; 
    cerr << "alpha_irs: " << rho_params->alpha_irs << endl;
    vector<double> mu(rho_params->mu);
    cerr << "mu size: " << mu.size() << endl;
...
double __find_rho (RhoParams* rho_params) {
    ...
    cerr << "reading from rho_params: " << rho_params->alpha_irs << endl;
...
int main() {
    ...
    for (auto v: mu) rho_params->mu.push_back(v);
    //rho_params->mu(mu);
    rho_params->alpha_irs = alpha_irs;
    __find_rho(rho_params);
    delete rho_params;
}
