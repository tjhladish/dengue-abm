...
ggsave(tail(args,1), plot=ggplot(intervention_examples.dt) + theme_minimal() + theme(
  ...
  scale_linetype_manual(
    ...
    labels=c(instant='all covered houses treated in 1 day', continuous='all covered houses treated in across 365 days', `90`='all covered houses treated across 90 days')
  ) +
  annotate("text",
    x = name.locs, y = mon.y,
